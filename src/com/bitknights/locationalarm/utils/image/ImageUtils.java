...
public class ImageUtils {
	...
	Matrix resizerMatrix = getResizerMatrix(bitmap.getWidth(), bitmap.getHeight(), size, size, true);
	return Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), resizerMatrix, true);
    }

    public static Matrix getResizerMatrix(float originalWidth, float originalHeight, float desiredWidth,
	...
	if ((desiredWidth == desiredHeight) || keepAspectRatio) {
	    float scaleMinValue = Math.min(desiredWidth / originalWidth, desiredHeight / originalHeight);
